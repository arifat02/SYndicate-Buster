...
<body>
        ...
        <div class="dashboard">
            ...
            <div class="userDetailsCard">
               ...
               <div class="navCard">
                ...
                <a href="../vendors/userInventory.php"style="background: rgba(255,255,255,0.1);">Product Registration</a>
                <a href="../vendors/sell_product.php">Make Sale</a>
    ...
    // Commodity unit data (would normally come from AJAX)
    const commodityUnits = {
        ...
        while($row = $unit_result->fetch_assoc()) {
            $unit_data[] = "'" . $row['commodity_id'] . "': '" . $row['unit_type'] . "'";
        }
    ...
    // Show modal message from PHP
    document.addEventListener('DOMContentLoaded', function() {
        showMessage('<?php echo $modal_type === 'warning' ? 'Warning' : 'Notice'; ?>', 
...
function showConfirmation(title, message, details, action, formFields = '') {
    ...
    // Open confirmation modal with higher z-index
...
function openModal(modalId) {
    ...
    function validateBatchForm() {
        ...
        const commodity = form.querySelector('[name="commodity"]');
        const quantity = form.querySelector('[name="quantity"]');
        const productionDate = form.querySelector('[name="production_date"]');
        ...
        if (!commodity.value || !quantity.value || !productionDate.value) {
            showMessage('Missing Information', 'Please fill all required fields', 'warning');
            return;
        }
        
        const commodityName = commodity.options[commodity.selectedIndex].text;
        const unitType = commodityUnits[commodity.value] || 'kg';
        ...
        const details = `
            ...
            <div class="confirmation-item">
                ...
                <span>${quantity.value} <span class="unit-display ${unitInfo.class}">${unitType}</span></span>
            </div>
            <div class="confirmation-item">
                <strong>Production Date:</strong>
                <span>${productionDate.value}</span>
            </div>
            ${form.querySelector('[name="expiry_date"]').value ? `
            <div class="confirmation-item">
                <strong>Expiry Date:</strong>
                <span>${form.querySelector('[name="expiry_date"]').value}</span>
            </div>` : ''}
        `;
        
        showConfirmation(
            'Confirm Add Batch',
            'Are you sure you want to add this batch?',
            details,
            'add_batch',
            '<input type="hidden" name="commodity" value="' + commodity.value + '">' +
            '<input type="hidden" name="quantity" value="' + quantity.value + '">' +
            '<input type="hidden" name="production_date" value="' + productionDate.value + '">' +
            '<input type="hidden" name="expiry_date" value="' + form.querySelector('[name="expiry_date"]').value + '">' +
            '<input type="hidden" name="parent_batch_id" value="' + form.querySelector('[name="parent_batch_id"]').value + '">' +
            '<input type="hidden" name="notes" value="' + form.querySelector('[name="notes"]').value + '">'
        );
    }
    ...
    function validateCommodityForm() {
        ...
        const unitType = form.querySelector('[name="unit_type"]');
        ...
        if (!name.value || !unitType.value || !category.value) {
            showMessage('Missing Information', 'Please fill all required fields', 'warning');
            return;
        }
        
        const categoryName = category.options[category.selectedIndex].text;
        const unitTypeName = unitType.options[unitType.selectedIndex].text;
        const isPerishable = form.querySelector('#perishable').checked;
        ...
        const details = `
            ...
            <div class="confirmation-item">
                ...
                <span>${isPerishable ? 'Yes' : 'No'}</span>
            </div>
            ${isPerishable ? `
            <div class="confirmation-item">
                <strong>Shelf Life:</strong>
                <span>${form.querySelector('[name="shelf_life_days"]').value} days</span>
            </div>` : ''}
        `;
        
        showConfirmation(
            'Confirm Add Commodity',
            'Are you sure you want to add this new commodity?',
            details,
            'add_commodity',
            '<input type="hidden" name="name" value="' + name.value + '">' +
            '<input type="hidden" name="unit_type" value="' + unitType.value + '">' +
            '<input type="hidden" name="category" value="' + category.value + '">' +
            '<input type="hidden" name="perishable" value="' + (isPerishable ? '1' : '0') + '">' +
            '<input type="hidden" name="shelf_life_days" value="' + form.querySelector('[name="shelf_life_days"]').value + '">' +
            '<input type="hidden" name="description" value="' + form.querySelector('[name="description"]').value + '">'
        );
    }
    
    // Show Edit Batch Confirmation
    function showEditBatchConfirmation(batchId, currentQty, initialQty, commodityName, unitType) {
        ...
        const details = `
            ...
            <div class="confirmation-item">
                ...
                <span>${currentQty} <span class="unit-display ${unitInfo.class}">${unitType}</span></span>
            </div>
            <div class="confirmation-item">
                <strong>Maximum Allowed:</strong>
                <span>${initialQty} <span class="unit-display ${unitInfo.class}">${unitType}</span></span>
            </div>
            <div class="confirmation-item">
            ...
            <div class="confirmation-item">
                ...
                <select name="reason" class="form-control">
                    ...
                    <option value="consumed">Self-Consumed</option>
    ...
    // Show Delete Batch Confirmation
    function showDeleteBatchConfirmation(batchId, commodityName) {
        ...
        showConfirmation(
            'Delete Batch',
            'Are you sure you want to permanently remove this batch?',
            details,
            'delete_batch',
            '<input type="hidden" name="batch_id" value="' + batchId + '">'
        );
